...
def roll_dice(num_rolls, dice=six_sided):
    ...
    "*** YOUR CODE HERE ***"
    Current_score = 0
    i = 1 # current times of calling dice
    sow_sad = 0
    while i <= num_rolls:
        Current_result = dice()
        if Current_result == 1:# SOW SAD
            sow_sad = 1
        Current_score, i = Current_score + Current_result, i + 1
    if sow_sad == 1:
        return 1
    else:
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    abs_difference = abs(player_score % 10 - (opponent_score // 10) % 10)
    if abs_difference * 3 > 1: # comparing abs_difference between player1's one digit and player2's ten digit with 1
...
def is_always_roll(strategy, goal=GOAL):
    ...
    while score < goal:
        ...
        while opp_score < goal:
            if first_strategy != strategy(score, opp_score):
                un_same = 1
            opp_score += 1
        score += 1
    if un_same == 1:
...
def make_averaged(original_function, times_called=1000):
    ...
    def target(*arg):
        ...
        while i <= times_called:
            sum_result = sum_result + original_function(*arg)
            i += 1
        return sum_result / times_called
    return target
    # END PROBLEM 8
